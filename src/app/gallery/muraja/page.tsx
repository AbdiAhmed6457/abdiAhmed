...
export default function MurajaGallery() {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/92 text-white flex flex-col">
      <header className="flex items-center justify-between p-4">
        <div className="text-lg font-semibold">Muraja — Full Gallery</div>
      ...
      <main className="flex-1 flex items-center justify-center px-4">
        <div className="relative w-full max-w-5xl h-[70vh] sm:h-[80vh]">
          <motion.div key={IMAGES[index]} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.45 }} className="absolute inset-0">
            <Image src={IMAGES[index]} alt={`Muraja ${index + 1}`} fill className="object-contain" priority sizes="(max-width: 1024px) 100vw, 60vw" />
          </motion.div>
